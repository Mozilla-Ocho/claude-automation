...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <issue_id> <image_file_path> [api_key]"
    echo "Example: $0 'CAR-1234' './screenshot.png'"
    exit 1
fi
...
# Step 3: Get current issue description
get_issue_mutation=$(cat <<EOF
{
...
issue_response=$(curl -s -X POST \
  -H "Content-Type: application/json" \
  -H "Authorization: $LINEAR_API_KEY" \
  -d "$get_issue_mutation" \
  https://api.linear.app/graphql)

current_description=$(echo "$issue_response" | jq -r '.data.issue.description // ""')

# Step 4: Append image to issue description
new_description="${current_description}

![${filename}](${asset_url})"

# Escape the description for JSON
escaped_description=$(echo "$new_description" | jq -Rs .)

# Step 5: Update issue with image
update_mutation=$(cat <<EOF
{
  "query": "mutation UpdateIssue(\$id: String!, \$description: String!) { issueUpdate(id: \$id, input: { description: \$description }) { success issue { id description updatedAt } } }",
  "variables": {
    "id": "$issue_id",
...
if echo "$response" | jq -e '.errors' > /dev/null 2>&1; then
    ...
    if [ "$success" = "true" ]; then
        echo "Image attached to issue description successfully!"
